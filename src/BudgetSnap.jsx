...
function AddTxSheet({ open, onClose, data, setData }) {
  ...
  const [cat, setCat] = useState(data.categories[0]?.id || "sonstiges");
  ...
  const save = () => {
    ...
    const updated = {
      ...data,
      transactions: [...data.transactions, { id: uid(), type, amount: amt, title: title.trim(), date, cat, note: note.trim() }],
    };
    setData(updated);
    saveData(updated);
    onClose();
